#!/bin/bash                                                                    
 ...
 if [ "$ENV_EXISTED" = true ]; then
     # Restore from our pre-deploy backup
     cp "$ENV_BACKUP" "$ENV_FILE"
     echo "  → Restored from backup"
 elif [ -f "$ENV_BACKUP" ]; then
     # .env gone somehow — try backup
     cp "$ENV_BACKUP" "$ENV_FILE"
     echo "  → Restored from backup"
 ...
 if [ "$BACKEND_OK" != true ]; then
     ...
     echo "    Run: docker compose -f $PROJECT_DIR/docker-compose.yml logs
 backend"
 fi
 ...
 CREATE INDEX IF NOT EXISTS idx_market_status_timestamp ON market_status_log
 (timestamp DESC);
 ...
 " > /dev/null 2>&1 && echo "  → Logs DB tables: OK" || echo "  → Logs DB
 tables: FAILED"
 ...
 REFRESH_OK=false
 for i in $(seq 1 5); do
     if curl -sf -X POST http://localhost:8001/api/v1/admin/refresh > /dev/null
  2>&1; then
         REFRESH_OK=true
